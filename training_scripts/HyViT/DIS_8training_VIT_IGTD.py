...
images_folder = "HyNNImages/Regression/images_8antennas_DIS_IGTD_40_40_no_inverted"
results_folder = "/results_ViT_IGTD_40_40_norm_1e-5/"
...
#df_x = combined_dataset.drop("homa_b",axis=1)
df_x = combined_dataset_x.drop("PositionX",axis=1)
df_y_for_x = combined_dataset_x["PositionX"]
...
# Assuming X_train["images"] contains the file paths of the images
first_image_path = X_train["images"][0]  # Get the first image path
first_image = cv2.imread(first_image_path)  # Read the first image

# Get dimensions
height, width, channels = first_image.shape 

# For 3 canal (RGB)
X_train_img = np.array([cv2.resize(cv2.imread(img),(height, width)) for img in X_train["images"]])
X_val_img = np.array([cv2.resize(cv2.imread(img),(height, width)) for img in X_val["images"]])
X_test_img = np.array([cv2.resize(cv2.imread(img),(height, width)) for img in X_test["images"]])
...
modelX = Model(inputs=[ff_model.input, vit_model.input], outputs=x)
...
modelY = Model(inputs=[ff_model.input, vit_model.input], outputs=x)
...
# VALIDATION RESULTS
folder = images_folder+"/predictions_ViT_IGTD_40_40_norm_1e-5/DIS/8antennas/validation/"
...
mae_x = mean_absolute_error(y_test_x, predX_val)
mse_x = mean_squared_error(y_test_x, predX_val)
rmse_x = mean_squared_error(y_test_x, predX_val, squared=False)
r2_x = r2_score(y_test_x, predX_val)

mae_y = mean_absolute_error(y_test_y, predY_val)
mse_y = mean_squared_error(y_test_y, predY_val)
rmse_y = mean_squared_error(y_test_y, predY_val, squared=False)
r2_y = r2_score(y_test_y, predY_val)
...
folder = images_folder+"/predictions_ViT_IGTD_40_40_norm_1e-5/DIS/8antennas/test/"
if not os.path.exists(folder):
    os.makedirs(folder)
