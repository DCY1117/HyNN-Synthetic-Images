...
results_folder = "/result_Only_MLP/"
...
x = Dense(32, activation="relu")(ff_model.output)
x = Dense(16, activation="relu")(x)
x = Dense(8, activation="relu")(x)
x = Dense(1, activation="linear")(x)

modelX = Model(inputs=[ff_model.input], outputs=x)
...
modelX.save(images_folder+'/predictions_Only_MLP/modelX_8.h5')
...
x = Dense(64, activation="relu")(ff_model.output)
x = Dense(32, activation="relu")(x)
x = Dense(16, activation="relu")(x)
x = Dense(8, activation="relu")(x)
x = Dense(1, activation="linear")(x)

modelY = Model(inputs=[ff_model.input], outputs=x)
...
modelY.save(images_folder+'/predictions_Only_MLP/modelY_8.h5')
...
# VALIDATION RESULTS
folder = images_folder+"/predictions_Only_MLP/DIS/8antennas/validation/"
if not os.path.exists(folder):
    os.makedirs(folder)

start_time = time.time()
predX_val = modelX.predict([X_val_num])
print("PREDICTION TIME OF X (VALIDATION): ", time.time()-start_time)

Start_time = time.time()
predY_val = modelY.predict([X_val_num])
...
start_time = time.time()
predictionX = modelX.predict([X_test_num])
print("PREDICTION TIME OF X (TEST): ", time.time()-start_time)

start_time = time.time()
predictionY = modelY.predict([X_test_num])
print("PREDICTION TIME OF Y (TEST): ", time.time()-start_time)

folder = images_folder+"/predictions_Only_MLP/DIS/8antennas/test/"
if not os.path.exists(folder):
    os.makedirs(folder)
